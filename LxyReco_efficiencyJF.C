...
void LxyReco_efficiencyJF() {
  ...
  while (myStdReader.Next()) {
  	...
  	for(int ij = 0; ij < jetPt->size(); ij++){
  		...
  		if(secVertB->at(ij) >= 0  && secVertB->at(ij) < 3 && jetFlav->at(ij) == 5) {
  			diff3 += sqrt((((float)jfNumTracks->at(ij)) - ((float)bHnumBtracks->at(ij))) * (((float)jfNumTracks->at(ij)) - ((float)bHnumBtracks->at(ij))));
				iter3++;
  		}
  		if(secVertB->at(ij) >= 3  && secVertB->at(ij) < 6 && jetFlav->at(ij) == 5) {
  			diff6 += sqrt((((float)jfNumTracks->at(ij)) - ((float)bHnumBtracks->at(ij))) * (((float)jfNumTracks->at(ij)) - ((float)bHnumBtracks->at(ij))));
				iter6++;
  		}
  		if(secVertB->at(ij) >= 6  && secVertB->at(ij) < 10 && jetFlav->at(ij) == 5) {
  			diff10 += sqrt((((float)jfNumTracks->at(ij)) - ((float)bHnumBtracks->at(ij))) * (((float)jfNumTracks->at(ij)) - ((float)bHnumBtracks->at(ij))));
				iter10++;
  		}
  		if(secVertB->at(ij) >= 10  && secVertB->at(ij) < 18 && jetFlav->at(ij) == 5) {
  			diff18 += sqrt((((float)jfNumTracks->at(ij)) - ((float)bHnumBtracks->at(ij))) * (((float)jfNumTracks->at(ij)) - ((float)bHnumBtracks->at(ij))));
				iter18++;
  		}
  		if(secVertB->at(ij) >= 18  && secVertB->at(ij) < 25 && jetFlav->at(ij) == 5) {
  			diff25 += sqrt((((float)jfNumTracks->at(ij)) - ((float)bHnumBtracks->at(ij))) * (((float)jfNumTracks->at(ij)) - ((float)bHnumBtracks->at(ij))));
				iter25++;
  		}
  		if(secVertB->at(ij) >= 25  && secVertB->at(ij) < 35 && jetFlav->at(ij) == 5) {
  			diff35 += sqrt((((float)jfNumTracks->at(ij)) - ((float)bHnumBtracks->at(ij))) * (((float)jfNumTracks->at(ij)) - ((float)bHnumBtracks->at(ij))));
				iter35++;
  		}
  		if(secVertB->at(ij) >= 35  && secVertB->at(ij) < 50 && jetFlav->at(ij) == 5) {
  			diff50 += sqrt((((float)jfNumTracks->at(ij)) - ((float)bHnumBtracks->at(ij))) * (((float)jfNumTracks->at(ij)) - ((float)bHnumBtracks->at(ij))));
				iter50++;
  		}
  	}
  }
  ...
  diff35 /= iter35;
  diff50 /= iter50;
  MultRecoEff_value -> SetPoint(0, 3, diff3);
  MultRecoEff_value -> SetPoint(1, 6, diff6);
  MultRecoEff_value -> SetPoint(2, 10, diff10);
  MultRecoEff_value -> SetPoint(3, 18, diff18);
  MultRecoEff_value -> SetPoint(4, 25, diff25);
  MultRecoEff_value -> SetPoint(5, 35, diff35);
  MultRecoEff_value -> SetPoint(6, 50, diff50);
  //MultRecoEffSV->Draw("colz");
  //MultRecoEffJF->Draw("colz");
  MultRecoEff_value->SetTitle("SV multiplicity reconstruction");
  MultRecoEff_value->SetLineColor(2);
  MultRecoEff_value->SetLineWidth(2);
  MultRecoEff_value->SetMarkerColor(4);
  MultRecoEff_value->SetMarkerSize(1.5);
  MultRecoEff_value->SetMarkerStyle(21);
  MultRecoEff_value->GetXaxis()->SetTitle("Distance from IP [mm]");
  MultRecoEff_value->GetYaxis()->SetTitle("Average |MultReco - MultGen|");
  MultRecoEff_value->Draw("SAME");
  TText * lineName = new TText(50, diff50, "JF");
